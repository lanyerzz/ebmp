...
<table id="table${pageId}"  class="easyui-datagrid"
    ...
    <tr>
        <th field="code" sortable="true">${message(code: 'bdQwsz.code.label', default: '编码')}</th>

        <th field="name" sortable="true">${message(code: 'bdQwsz.name.label', default: '名称')}</th>
        <th field="bdInvmandoc.pkInvbasdoc.invcode" sortable="true" ><g:message code="bdQwsz.bdInvmandoc.invcode.label" default="存货编码"/></th>
        <th field="bdInvmandoc.pkInvbasdoc.invname" sortable="true" ><g:message code="bdQwsz.bdInvmandoc.invname.label" default="存货名称"/></th>
        <th field="canUse" sortable="true" data-options="align:'center',formatter:function(value,row){
                    if(row.iscustomer==true){
                        return '${g.message(code: 'default.boolean.true')}';
                    }else{
                        return '${g.message(code: 'default.boolean.false')}';
                    }
                }">${message(code: 'bdQwsz.canUse.label', default: '是否可用')}</th>
        <th field="state" sortable="true" data-options="align:'center',formatter:function(value,row){
                    if(row.iscustomer==true){
                        return '${g.message(code: 'default.state.open')}';
                    }else{
                        return '${g.message(code: 'default.state.close')}';
                    }
                }">${message(code: 'bdQwsz.state.label', default: '状态')}</th>
        <th field="note" sortable="true">${message(code: 'bdQwsz.note.label', default: '备注')}</th>
        <th field="bdCorp.unitName" sortable="false"><g:message code="bdQwsz.bdCorp.label" default="公司"/></th>
    </tr>
    </thead>
</table>
