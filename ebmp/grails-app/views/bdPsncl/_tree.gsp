...
<table id="table${pageId}" class="easyui-treegrid" style="width:400px;height:300px"
    ...
    <tr>
        ...
        <th field="code" sortable="false">${message(code: 'bdPsncl.code.label', default: 'Code')}</th>

        <th field="name" sortable="false">${message(code: 'bdPsncl.name.label', default: 'Name')}</th>
        ...
        }"><g:message code="bdPsncl.bdCorp.label" default="Bd Corp"/></th>

        <th field="psnclscope" sortable="false" data-options="formatter:function(value,row){
            if(row.psnclscope==0){
                return '在职';
            }else if(row.psnclscope==1){
                return '实习期';
            }else if(row.psnclscope==2){
                return '离职';
            }else{
               return value;
            }
        }">${message(code: 'bdPsncl.psnclscope.label', default: 'Psnclscope')}</th>

        <th field="sealflag" sortable="false" data-options="formatter:function(value,row){
            if(row.sealflag=='true'){
                return '是';
            }else{
                return '否';
            }
        }">${message(code: 'bdPsncl.sealflag.label', default: 'Sealflag')}</th>

        <th field="memo" sortable="false">${message(code: 'bdPsncl.memo.label', default: 'Memo')}</th>
