...
<input id="${id}${pageId}"   name="${name}" data-options="
                  {
                   <g:if test="${value}">url:'${createLink(controller: 'baseContract',action: 'queryAll')}/${value}',</g:if>
<g:else> onShowPanel:function(){ _onShowPanelInForm('${id}','${pageId}','${createLink(controller: 'baseContract',action: 'query')}'); }, </g:else>
...
<g:else>
    <th field="${field}" sortable="false" width="100" editor="{type:'comboxgrid',options:{
                 ...
                 mode:'remote',
                  ...
                  rownumbers:true,singleSelect:true,pageSize:10,
                  idField:'id',textField:'contractNo',
                url:'${createLink(controller: 'baseContract',action: 'query')}',
                <g:if test="${onSelect}"> onSelect:${onSelect},</g:if>
    <g:if test="${onChange}"> onChange:${onChange},</g:if>
    onShowPanel:function(){
      _onShowPanel('${field}','${pageId}','${createLink(controller: 'baseContract',action: 'query')}');
                                    },
                columns:[[
