...
<script>
    ...
    function  onShow${pageId}(){
        ...
        var showRrl='${createLink(action:'show')}/'+row.id

        showCard(showRrl,'${pageId}');
...
//
    ...
    function onRefPage${pageId}(businessKey,billTypeKey,title,targetBill){
        ...
        function onOut${pageId}(){

            var row=$('#table${pageId}').datagrid('getSelected');
            if(row == null){//选择修改节点

                $.messager.show({title:'警告',msg:'请先选择数据'});
                return;
            }
            var  href='${createLink(action: 'outPage')}?headId='+row.id;
             showCard( href,'${pageId}');
        }

        function onIn${pageId}(){
            var row=$('#table${pageId}').datagrid('getSelected');
            if(row == null){//选择修改节点
            ...
            var  href='${createLink(action: 'inPage')}?headId='+row.id;
            showCard( href,'${pageId}');
        }
...
<div id="main${pageId}" style="height: 100%;padding: 3px" class="easyui-panel" data-options="fit:true,border:false,noheader:true,onResize:onResize${pageId}"  >
    <div id="list${pageId}" class="easyui-panel" title=""
        ...
        <a href="#" class="easyui-linkbutton"  plain="true" onclick="onOut${pageId}()">
            ${message(code: 'default.button.ck.label',default: '出库')}
        </a>
        <a href="#" class="easyui-linkbutton"  plain="true" onclick="onIn${pageId}()">
            ${message(code: 'default.button.rk.label',default: '入库')}
        </a>
        </div>
