<script>
    ...
    function change${pageId}(new_,old_){
        ...
        $('#notAllotedList${pageId}').datagrid('reload',{'pkInvcl':'${invclId}','pkCorp':new_});
        $('#allotedList${pageId}').datagrid('reload',{'pkInvcl':'${invclId}','pkCorp':new_});
    }
    ...
    function onAllot${pageId}(){
        ...
        $.post('${createLink(action:'allot')}',{'pkInvbasdoc':row.id,'pkCorp':corp},function(json){
            if(json.success){
    ...
    function onAllotAll${pageId}(){
        ...
        $.post('${createLink(action:'allotAll')}',{'pkCorp':corp,'pkInvcl':invcl},function(json){
            if(json.success){
    ...
    function onCancelAllot${pageId}(){
        ...
        $.post('${createLink(action:'deleteAllot')}',{'pkInvbasdoc':row.id,'pkCorp':corp},function(json){
            if(json.success){
    ...
    function onCancelAllotAll${pageId}(){
        ...
        $.post('${createLink(action:'deleteAllotAll')}',{'pkCorp':corp,'pkInvcl':invcl},function(json){
            if(json.success){
                $('#allotedList${pageId}').datagrid('reload');
                $('#notAllotedList${pageId}').datagrid('reload');
                $.messager.show({
...
<div class="easyui-layout" fit="true">
    <div region="north" border="false" style="height:35px">
        <div id="corp${pageId}" class="fieldcontain  ">
            <label for="corp">
                <g:message code="bdInvbasdoc.pkCorp.label" default="Pk Corp" />
    ...
    <div region="west" style="width:245px">
        <table id="notAllotedList${pageId}" class="easyui-datagrid" url="${createLink(action:'queryInvbasdocNotInCorp')}" fit="true" singleSelect="true">
            <thead>
                <tr>
                    <th field="invcode" sortable="true"><g:message code="bdInvbasdoc.invcode.label"></g:message></th>
                    <th field="invname" sortable="true"><g:message code="bdInvbasdoc.invname.label"></g:message></th>
                </tr>
            </thead>
        </table>
    </div>
    ...
    <div region="east" style="width:245px">
        <table id="allotedList${pageId}" class="easyui-datagrid" url="${createLink(action:'queryInvbasdocInCorp')}" fit="true" singleSelect="true">
            <thead>
            <tr>
                <th field="invcode" sortable="true"><g:message code="bdInvbasdoc.invcode.label"></g:message></th>
                <th field="invname" sortable="true"><g:message code="bdInvbasdoc.invname.label"></g:message></th>
