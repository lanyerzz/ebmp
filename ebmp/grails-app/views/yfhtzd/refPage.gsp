<script>
    ...
    function getHeadData(){
        ...
        if(data.length==0){
            $.messager.show({
                title:'警告',
                msg:'请选择${message(code: 'yfhtzd.label', default: 'Yfhtzd')}主信息!'
            });
            return false;
        }
    ...
    function ${handler}() {
        ...
        if (data.length == 0) {
            $.messager.show({
                title: '警告',
                msg: '请选择${message(code: 'yfhtzd.label', default: 'Yfhtzd')}明细数据!'
            });
...
<g:render template="list" model="[targetBill: targetBill,businessType:businessKey]"></g:render>