...
<script>
  ...
  function  onEditNum${pageId}(){
      $("#numEditDlg${pageId}").dialog('open');
      var obj= $('#form${pageId}').serializeObject();
      $('#pz1${pageId}').numberbox('setValue',obj.pz);
      $('#mz1${pageId}').numberbox('setValue',obj.mz);
      $('#fysl1${pageId}').numberbox('setValue',obj.fysl);
  ...
  function  onShow${pageId}(){
    ...
    $.post( '${createLink(action: 'save')}', {'head':JSON.stringify(head),'bodys':JSON.stringify(bodys)},function(json){
            ...
            if($('#card${pageId}').showMessage(json)){//有错误数据
                ...
                $.post('${createLink(controller: 'yfhtzd',action: 'queryCarIning')}',null,function(json){
...
//                        '/Users/lanyer/git/ebmp/ebmp/web-app/test.txt'
                    if(!getCookie('jh_file_path')){
                        $.messager.alert('警告','请先在[发货通知单]节点设置选项里设置文件路径,否则无法清除屏幕叫号信息.');
                        return;
                    }
...
function onNumEdit${pageId}(){
    $("#numEditDlg${pageId}").dialog('close');
    var obj= $('#numForm${pageId}').serializeObject();
//    alert(JSON.stringify(obj));
    $('#pz${pageId}').numberbox('setValue',obj.pz);
    $('#mz${pageId}').numberbox('setValue',obj.mz);
    $('#fysl${pageId}').numberbox('setValue',obj.fysl);
...
<div id="numEditBt${pageId}">
    <a href="#" class="easyui-linkbutton"  plain="true" iconCls="icon-edit" onclick="onNumEdit${pageId}()">
        ${message(code: 'default.button.edit.label')}
    </a>
</div>

<div id="numEditDlg${pageId}" class="easyui-dialog" data-options=" resizable:false,
            title:'重量调整',
            modal:true,
            width:800,
            height:300,buttons:'#numEditBt${pageId}',closed:true">
    <g:render template="numEdit"></g:render>
</div>
...
<div id="main${pageId}" style="height: 100%;padding: 3px" class="easyui-panel" data-options="fit:true,border:false,noheader:true,onResize:onResize${pageId}"  >
    <div id="list${pageId}" class="easyui-panel" title=""
        ...
        <a href="#" class="easyui-linkbutton"  plain="true" iconCls="icon-uncheck" onclick="onUnCheck${pageId}()">
            ${message(code: 'default.button.uncheck.label')}
        </a>
