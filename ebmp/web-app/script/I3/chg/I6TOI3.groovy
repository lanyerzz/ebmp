...
head.billTypeCode='I3'
head.billtype=PubBilltype.findByBilltypecode('I3')
...
data?.each {
    ...
    body.hsl=b.hsl
    body.ninNum= it.nowinnum?.toBigDecimal()
    body.ninassistNum=it.nowinastnum?.toBigDecimal()
    body.sourceBid=b.id
    body.sourceHid=b.head.id
    body.sourceBillCode=b.head.billTypeCode
    body.sourceBillType=b.head.billtype
    body.nshouldinNum=it.nowinnum?.toBigDecimal()
    body.nneedinassistNum=it.nowinastnum?.toBigDecimal()
    head.addToBody(body)
